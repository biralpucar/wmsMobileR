...
interface AmbarCikisSiparisBaglantiliFormProps {
  formData: {
    selectedDepo?: string;
    hucreTuru?: string;
    cikisYeri?: string;
...
const AmbarCikisSiparisBaglantiliForm: React.FC<AmbarCikisSiparisBaglantiliFormProps> = ({ formData, onBack, onComplete }) => {
  // Dil hook'u
  const { t } = useLanguage();
  ...
  const productCategories = [
    {
      ...
      products: [
        { id: 1, name: 'Vidalı Kompresör', stokKodu: 'STK001', miktar: 10, seriTakibi: 'Evet', stock: '10 adet', miktarKadarSeri: 'Evet' },
        { id: 2, name: 'Rulman 6205 ZZ', stokKodu: 'STK002', miktar: 50, seriTakibi: 'Hayır', stock: '50 adet', miktarKadarSeri: 'Hayır' }
      ]
    },
    {
      id: 2,
      ...
      products: [
        { id: 3, name: 'Yağ Filtresi', stokKodu: 'STK003', miktar: 20, seriTakibi: 'Evet', stock: '20 adet', miktarKadarSeri: 'Hayır' }
      ]
    },
    {
      id: 3,
      ...
      products: [
        { id: 4, name: 'Hava Filtresi', stokKodu: 'STK004', miktar: 15, seriTakibi: 'Hayır', stock: '15 adet', miktarKadarSeri: 'Hayır' }
      ]
    },
    {
      id: 4,
      ...
      products: [
        { id: 5, name: 'Kompresör Kayışı', stokKodu: 'STK005', miktar: 5, seriTakibi: 'Hayır', stock: '5 adet', miktarKadarSeri: 'Hayır' }
      ]
    },
    {
      id: 5,
      ...
      products: [
        { id: 6, name: 'Elektronik Kart', stokKodu: 'STK006', miktar: 2, seriTakibi: 'Evet', stock: '2 adet', miktarKadarSeri: 'Hayır' }
      ]
    }
  ];
  ...
  const handleSubmit = async () => {
    ...
    if (!hasSelectedCategory) {
      setAlertConfig({ title: 'Hata', message: 'Lütfen en az bir kategori seçiniz.', type: 'error' });
    ...
    setTimeout(() => {
      ...
      // onComplete prop'unu çağır (AmbarCikisSiparisBaglantiliForm2'ye yönlendir)
                      ...
                      style={{
                        padding: 16,
                         ...
                         backgroundColor: index % 2 === 0 
                           ? grayColor
                           : greenColor,
                         borderBottomWidth: index < category.products.length - 1 ? 1 : 0,
                         borderBottomColor: borderColor
                      }}
                    >
                      <View style={{ flex: 1 }}>
                        <Text style={{fontSize: 16, fontWeight: '500', color: textColor, marginBottom: 4}}>{product.name}</Text>
                        <Text style={{fontSize: 14, color: textColor, marginBottom: 4}}>{t('stokKodu')}: {product.stokKodu}</Text>
                        <Text style={{fontSize: 14, color: textColor, marginBottom: 4}}>{t('miktar')}: {product.miktar}</Text>
                        <Text style={{fontSize: 14, color: textColor, marginBottom: 4}}>{t('stok')}: {product.stock}</Text>
                        <Text style={{fontSize: 14, color: textColor, marginBottom: 4}}>{t('seriTakibi')}: {product.seriTakibi}</Text>
                        <Text style={{fontSize: 14, color: textColor}}>{t('miktarKadarSeri')}: {product.miktarKadarSeri}</Text>
                      </View>
                    </View>
                  ))}
                </View>
...
export default AmbarCikisSiparisBaglantiliForm;